...
`jooservices/flickr` is an owner-driven project (benevolent-leader style). JOOservices maintains the package; the project owner holds final decision authority.
...
- Breaking changes are acceptable only in major versions; this line starts at `v4.0.0` as a full rebuild with no backward compatibility with v2

## Quality authority

The repository quality gates (Pint, PHPCS, PHPStan max level with strict rules and no ignores, PHPMD, PHP-CS-Fixer, 85% coverage floor, required Docker CI chain) may only be relaxed by the owner.
